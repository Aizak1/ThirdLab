...
namespace SecondTask
{
    class Task2Logic
    {
        ...
        public Task2Logic(double _initialX,double _eventualX,double _dx, double _accuracy)
        {
        ...
        private void CalculateTablesValue()
        {
            ...
            for (double i = _initialX; Math.Round(i,6) <= (float)_eventualX; i += (float)_dx)
            {
                ...
                {
                    currentMember = Math.Pow(-1, (double)(n + 1)) / (double)((2 * n + 1) * (Math.Pow(i, (double)(2 * n + 1))));
                    y += currentMember;
                }
                Arguments[counter] = i;
                FunctionValuesWithRaws[counter] = y;
                Steps[counter] = n;
                FunctionValues[counter] = Math.Atan(i);
                counter++;
            }
